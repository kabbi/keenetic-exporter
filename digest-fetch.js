...
class DigestClient {
  ...
  addAuth(url, options) {
    ...
    const response = cryptojs
      .MD5(
        `${ha1}:${this.digest.nonce}:${ncString}:${this.digest.cnonce}:${
          this.digest.qop
        }:${ha2}`,
      )
      .toString();
    const opaqueString = this.digest.opaque
      ...
      : '';
    const digest = `${this.digest.scheme} username="${this.user}",realm="${
      this.digest.realm
    }",\
nonce="${this.digest.nonce}",uri="${uri}",${opaqueString}\
qop=${
      this.digest.qop
    },algorithm="{this.digest.algorithm}",response="${response}",nc=${ncString},cnonce="${
      this.digest.cnonce
    }"`;
